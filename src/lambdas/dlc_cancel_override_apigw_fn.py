...
CANCEL_OVERRIDE_ALERT_SUBJECT_FORMAT = (
    "Load Control Cancel Override has Failed - {hint}"
)
...
def initiate_step_function(correlation_id: str, request: dict) -> dict:
    ...
    timestamp: str = datetime.now(timezone.utc).strftime(
        CANCELLATION_EXECUTION_ID_DATETIME_FORMAT
    )
    step_function_execution_id: str = f"{correlation_id}-{timestamp}"

    sm_handler = StateMachineHandler(
        STEP_FUNCTION_CLIENT, AppConfig.DLC_CANCEL_OVERRIDE_SM_ARN
    )
    ...
    if HTTPStatus.OK == status_code:
        ...
        logger.info(
            "Invoked SM to process DLC request. ARN: %s, StartDateTime: %s",
        ...
        raise RuntimeError(
            f"Launching DLC cancel request step function resulted in {status_code} status code"
        )
...
def process_request(event: dict) -> dict:
    ...
    logger.info(
        "Subscription id: %s, Correlation id: %s, Subscriber: %s",
    ...
    try:
        ...
        (
            subscription_record,
            subscription_errors,
        ) = RequestValidator.validate_subscription(
            provided_subscription_id, LOAD_CONTROL_SERVICE_NAME
        )

        if subscription_errors:
            raise InvalidRequest(
                f"Subscription id {provided_subscription_id} is not valid"
            )
        ...
        if header_record.get("group_id", None):
            raise InvalidRequest(
                f"Correlation id {correlation_id} is a part of group dispatch and cannot be cancelled"
            )
        ...
        if current_stage not in IN_PROGRESS_STAGES:
            raise InvalidRequest(
                f"Load control request in state: {current_stage} - cannot cancel from this state"
            )
        ...
        if datetime.fromisoformat(header_record.get("rqstEndDt")) < now:
            raise InvalidRequest(
                "Request given has an end date in the past so is already completed"
            )
        ...
        return format_response(
            ...
            {"message": str(e), "correlation_id": correlation_id},
        )
        ...
        if "ExecutionDoesNotExist" == e.__class__.__name__:
            ...
            subject = CANCEL_OVERRIDE_ALERT_SUBJECT_FORMAT.format(
                hint="Correlation ID Not Found"
            )
            reason = f"Correlation id {correlation_id} not found"
        else:
            subject = CANCEL_OVERRIDE_ALERT_SUBJECT_FORMAT.format(
                hint="Client Internal Error"
            )
        ...
        return format_response(
            ...
            {"message": reason, "correlation_id": correlation_id},
        )
...
if __name__ == "__main__":
    data: dict = {
        "pathParameters": {"subscription_id": "2d45fe05f3e44be38673b9ffd11fa2db"},
