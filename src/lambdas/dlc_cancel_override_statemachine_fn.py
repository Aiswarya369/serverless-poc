...
from src.utils.tracker_utils import update_tracker, get_header_record, get_contiguous_request
...
STEP_FUNCTION_IN_PROGRESS_STAGES = [Stage.QUEUED.value, Stage.POLICY_CREATED.value, Stage.POLICY_EXTENDED.value]
POLICY_DEPLOYED_STATES = [Stage.POLICY_DEPLOYED.value, Stage.DLC_OVERRIDE_STARTED.value, Stage.EXTENDED_BY.value]
...
def evaluate_request(event: dict) -> dict:
    ...
    elif request["current_stage"] in [Stage.POLICY_EXTENDED.value, Stage.POLICY_DEPLOYED.value,
                                      Stage.DLC_OVERRIDE_STARTED.value]:
        contiguous_request, _ = get_contiguous_request(request)

        # Contiguous with the same switch direction
        if contiguous_request and request['status'] == contiguous_request['overrdValue']:
            contiguous_request_start_date = datetime.fromisoformat(contiguous_request['rqstStrtDt'])
            contiguous_request_end_date = datetime.fromisoformat(contiguous_request['rqstEndDt'])
            ...
            logger.info("Contiguous request start date %s\nend date %s\nNow %s",
            ...
            # will not be deployed until after the contiguous request has started.
                ...
                update_tracker(
                    ...
                    request_start_date=datetime.fromisoformat(contiguous_request["rqstStrtDt"]),
                    request_end_date=datetime.fromisoformat(contiguous_request["rqstEndDt"])
                )
...
def create_policy(payload: dict) -> dict:
    ...
    start_datetime: datetime = datetime.fromisoformat(evaluated_request['replacement_start_datetime'])
    end_datetime: datetime = datetime.fromisoformat(evaluated_request['replacement_end_datetime'])
    ...
    if current_stage in STEP_FUNCTION_IN_PROGRESS_STAGES:
        execution_arn: str = f"{AppConfig.DLC_OVERRIDE_SM_EXECUTION_ARN}:{request['correlation_id']}"
    ...
    policy_name, response = policynet_client.create_lc_override_schedule_policy(meter_serial, turn_off, start_datetime,
                                                                                duration, replace)
    ...
    if status_code != HTTP_SUCCESS:
        ...
        raise RuntimeError("Error creating policy; PolicyNet returned response: %s", response)
    else:
        # update tracker
        update_tracker(
...
def cancel_complete(payload: dict) -> None:
    ...
    # Update tracker.
    logger.info("Updating load control request tracker to CANCELLED with correlation id %s at %s",
                correlation_id, stopped_datetime)
    update_tracker(
    ...
    # Deliver to kinesis
    payload: dict = assemble_event_payload(correlation_id, Stage.CANCELLED, stopped_datetime, CANCELLATION_REASON)
...
def failure_handler(payload: dict):
    ...
    logger.error("DLC cancel override step function failed with the following error: %s\n cause: %s", error, cause)
...
def lambda_handler(event: dict, _):
    ...
    logger.info("DLC cancel override step function lambda started with event: %s", event)
...
if __name__ == "__main__":
    ...
    # Cancel policy
    cancel_payload = {'action': 'cancelPolicy', 'payload': {
        'request': {'correlation_id': '4407114520-2022-07-12T135313-70a089e7-0d9c-488e-aaea-ec4892552c00',
                    'site': '4407114520', 'switch_addresses': ['LG022102554'], 'status': 'ON',
                    'meter_serial_number': 'LG022102554', 'current_stage': 'QUEUED',
                    'start_datetime': '2022-07-26T21:13:00+00:00', 'end_datetime': '2022-07-26T21:17:00+00:00',
                    'policy_id': None, 'extended_by': None},
        'evaluated_request': {
            'payload': {'cancelled_correlation_id': '4407114520-2022-07-12T135313-70a089e7-0d9c-488e-aaea-ec4892552c00',
                        'policy_id': None, 'replace': False}}}}
