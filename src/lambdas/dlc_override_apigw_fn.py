...
OVERRIDE_THROTTLING_QUEUE = os.environ.get("OVERRIDE_THROTTLING_QUEUE", "load-control-throttle-queue")
...
def format_response(status_code: HTTPStatus, body: Union[Dict[str, Any], str]) -> Dict[str, Any]:
    """
...
def add_request_on_throttle_queue(correlation_id: str, request: Dict[str, Any]) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if status_code == HTTPStatus.OK:
            ...
            return format_response(HTTPStatus.OK, {
...
def report_errors(correlation_id: str, error_datetime: datetime, error_message: Union[str, List[ValidationError]]):
    """
...
def job_entry(event: Dict[str, Any]) -> Dict[str, Any]:
    ...
    errors = RequestValidator.validate_dlc_override_request(request, DEFAULT_OVERRIDE_DURATION_MINUTES)
    if errors:
    ...
    _, subscription_errors = RequestValidator.validate_subscription(subscription_id, LOAD_CONTROL_SERVICE_NAME)
    ...
    duration_errors = RequestValidator.validate_request_duration(request, DEFAULT_OVERRIDE_DURATION_MINUTES)
    if duration_errors:
...
def lambda_handler(event: Dict[str, Any], _context: Any):
    ...
    try:
        ...
        logger.exception("Failed to process DLC request event. Event: %s, Error: %s", str(event), repr(e), exc_info=e)
        # todo
...
if __name__ == "__main__":
    data: dict = {
        "pathParameters": {
            "subscription_id": "a25fb76c298447e3996281098c59ed00"
        },
